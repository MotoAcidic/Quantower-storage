# Build Ocean's Read and drop it where ATAS loads custom indicators.
...
dotnet build (Join-Path $root 'OceansRead.csproj') -c Release
if ($LASTEXITCODE -ne 0) { throw 'Build failed.' }

# A constructor that throws is invisible inside ATAS - the indicator simply never appears in
# the list - so it is caught out here instead. The output is captured whole rather than piped:
# cutting a pipeline short with Select-Object stops the child process and leaves $LASTEXITCODE
...
$dll = Join-Path $root 'bin\Release\OceansRead.dll'
try {
    Copy-Item $dll (Join-Path $target 'OceansRead.dll') -Force
}
catch {
