...
namespace OceansRead
{
    ...
    public sealed class TimeContext
    {
                                         ...
                                         DateTime nowUtc, int haltHour)
        {
            ...
            {
                return Fail($"Ocean Read: no such time zone '{zoneId}' ({ex.Message}). " +
            ...
            if (bars == null || bars.Count == 0)
                return Fail("Ocean Read: no bars loaded yet.");

            var found = ByWallClock(bars, zone, nowUtc, out var method)
                     ?? ByDailyHalt(bars, zone, haltHour, out method);

            if (found == null)
                return Fail("Ocean Read: could not tell whether bar times are UTC or " +
