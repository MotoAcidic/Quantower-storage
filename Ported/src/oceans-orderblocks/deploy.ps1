...
dotnet build (Join-Path $root 'OceansOrderBlocks.csproj') -c Release -p:SkipAtasDeploy=true
...
$smokeOutput | Select-Object -Last 1

if (-not (Test-Path $target)) { New-Item -ItemType Directory -Path $target | Out-Null }

$dll = Join-Path $root 'bin\Release\OceansOrderBlocks.dll'
try {
    Copy-Item $dll (Join-Path $target 'OceansOrderBlocks.dll') -Force
}
catch {
