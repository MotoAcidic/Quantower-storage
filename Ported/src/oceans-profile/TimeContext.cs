...
namespace OceansProfile
{
    ...
    public sealed class TimeContext
    {
                                         ...
                                         DateTime nowUtc, int haltHour)
        {
            ...
            {
                return Fail($"Ocean Profile: no such time zone '{zoneId}' ({ex.Message}). " +
            ...
            if (bars == null || bars.Count == 0)
                return Fail("Ocean Profile: no bars loaded yet.");

            var found = ByWallClock(bars, zone, nowUtc, out var method)
                     ?? ByDailyHalt(bars, zone, haltHour, out method);

            if (found == null)
                return Fail("Ocean Profile: could not tell whether bar times are UTC or " +
