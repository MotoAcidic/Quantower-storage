...
Write-Host 'Running profile tests...' -ForegroundColor Cyan
Push-Location (Join-Path $root '_test')
try { dotnet run -c Release | Select-Object -Last 1 }
finally { Pop-Location }
if ($LASTEXITCODE -ne 0) { throw 'Profile tests failed - not deploying.' }

Write-Host 'Building...' -ForegroundColor Cyan
dotnet build (Join-Path $root 'OceansProfile.csproj') -c Release
...
$dll = Join-Path $root 'bin\Release\OceansProfile.dll'
try {
    Copy-Item $dll (Join-Path $target 'OceansProfile.dll') -Force
}
catch {
