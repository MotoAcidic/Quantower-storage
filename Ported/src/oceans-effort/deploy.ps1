# Build Ocean Effort and drop it where ATAS loads custom indicators.
...
Write-Host 'Running effort tests...' -ForegroundColor Cyan
Push-Location (Join-Path $root '_test')
try { $tests = dotnet run -c Release }
finally { Pop-Location }
if ($LASTEXITCODE -ne 0) {
    $tests | Write-Host
    throw 'Effort tests failed - not deploying.'
}
$tests | Select-Object -Last 1 | Write-Host

Write-Host 'Building...' -ForegroundColor Cyan
dotnet build (Join-Path $root 'OceansEffort.csproj') -c Release
...
$dll = Join-Path $root 'bin\Release\OceansEffort.dll'
try {
    Copy-Item $dll (Join-Path $target 'OceansEffort.dll') -Force
}
catch {
