# Build Ocean SMA and drop it where ATAS loads custom indicators.
...
Write-Host 'Running SMA tests...' -ForegroundColor Cyan
Push-Location (Join-Path $root '_test')
try { dotnet run -c Release | Select-Object -Last 1 }
finally { Pop-Location }
if ($LASTEXITCODE -ne 0) { throw 'SMA tests failed - not deploying.' }

Write-Host 'Building...' -ForegroundColor Cyan
dotnet build (Join-Path $root 'OceansSma.csproj') -c Release
if ($LASTEXITCODE -ne 0) { throw 'Build failed.' }

if (-not (Test-Path $target)) { New-Item -ItemType Directory -Path $target | Out-Null }

$dll = Join-Path $root 'bin\Release\OceansSma.dll'
try {
    Copy-Item $dll (Join-Path $target 'OceansSma.dll') -Force
}
catch {
