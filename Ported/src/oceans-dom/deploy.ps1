# Build Ocean DOM and drop it where ATAS loads custom indicators.
...
Write-Host 'Running DOM tests...' -ForegroundColor Cyan
Push-Location (Join-Path $root '_test')
try { dotnet run -c Release | Select-Object -Last 1 }
finally { Pop-Location }
if ($LASTEXITCODE -ne 0) { throw 'DOM tests failed - not deploying.' }

Write-Host 'Building...' -ForegroundColor Cyan
dotnet build (Join-Path $root 'OceansDom.csproj') -c Release
...
$dll = Join-Path $root 'bin\Release\OceansDom.dll'
try {
    Copy-Item $dll (Join-Path $target 'OceansDom.dll') -Force
}
catch {
