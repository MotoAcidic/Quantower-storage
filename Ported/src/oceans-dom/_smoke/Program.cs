...
// invisible in the platform -- the indicator simply never appears -- so it is worth catching
// here, where the exception is actually readable.
//
...
static class Program
{
    ...
    static int Run()
    {
        try
        {
            var indicator = new OceansDom.OceansDomIndicator();
            ...
            // The levels setting is the one place a typo can cost a level you think you are
            // watching, so the parse is exercised here too rather than only in the math harness.
            var parsed = OceansDom.LevelParser.Parse("PDH=29650.25, PDL 29500, ORB-H=29612.5, bad entry");
            Console.WriteLine();
            Console.WriteLine("Levels parse: " + parsed.Levels.Count + " read, " +
                              parsed.Unreadable.Count + " unreadable");
            ...
            foreach (var property in indicator.GetType().GetProperties(BindingFlags.Public | BindingFlags.Instance))
            {
                ...
                if (property.DeclaringType != typeof(OceansDom.OceansDomIndicator)) continue;
