...
namespace OceansAnchor
{
    ...
    /// silently misplaced set of HVN shelves, and an A+ signal fired at a fictional level is
    /// worse than no signal at all. So it never guesses: it either settles the clock from
    /// evidence or reports Error, and the indicator draws that instead of zones.
    /// </summary>
    public sealed class BarClockContext
    {
                                             ...
                                             TimeSpan barDuration)
        {
            ...
            {
                return Fail("Ocean's Anchor: no such time zone " + zoneId + " (" + ex.Message +
            ...
            if (bars == null || bars.Count == 0)
                return Fail("Ocean's Anchor: no bars loaded yet.");
        ...
        private static string Explain(IBarWindow bars, TimeZoneInfo zone, TimeSpan barDuration)
        {
            ...
            var msg = "Ocean's Anchor: cannot settle the bar clock. Newest bar is stamped " +
