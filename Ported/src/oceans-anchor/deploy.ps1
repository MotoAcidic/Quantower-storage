...
dotnet build (Join-Path $root 'OceansAnchor.csproj') -c Release -p:SkipAtasDeploy=true
...
$dll = Join-Path $root 'bin\Release\OceansAnchor.dll'
try {
    Copy-Item $dll (Join-Path $target 'OceansAnchor.dll') -Force
}
catch {
