...
namespace OceansCurrent
{
    ...
    public sealed class TimeContext
    {
                                         ...
                                         DateTime nowUtc, int haltHour)
        {
            ...
            {
                return Fail($"Ocean's Current: no such time zone '{zoneId}' ({ex.Message}). " +
            ...
            if (bars == null || bars.Count == 0)
                return Fail("Ocean's Current: no bars loaded yet.");

            var found = ByWallClock(bars, zone, nowUtc, out var method)
                     ?? ByDailyHalt(bars, zone, haltHour, out method);

            if (found == null)
                return Fail("Ocean's Current: could not tell whether bar times are UTC or " +
        ...
        /// (3-4 PM Houston). Under the right reading that hour is empty on every single day,
    ...
    public interface IBarWindow
    {
        ...
        decimal Volume(int bar);

        /// <summary>Ask-traded minus bid-traded for the bar. Zero on a feed without a tape.</summary>
        decimal Delta(int bar);
    }
}
