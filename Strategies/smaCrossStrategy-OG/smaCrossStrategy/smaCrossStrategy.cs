...
namespace SimpleMACross
{
    public sealed class SimpleMACross : Strategy, ICurrentAccount, ICurrentSymbol
    {
        ...
        // Set true when a bar completes; entry logic consumes and resets it
        ...
        private void Core_PositionRemoved(Position obj)
        {
            ...
            if (!positions.Any())
            {
                ...
                // Cancel any remaining SL/TP orders so they don't trigger new positions
        ...
        {
            // A bar just completed — allow one entry on the very next HistoryItemUpdated tick.
            // We do NOT call OnUpdate() here because GetValue(0) at this instant is the new
            // forming bar (possibly empty), which would cause spurious exit and entry signals.
            this.newBar = true;
        }
        ...
        private void OnUpdate()
        {
            ...
            {
                // Gate entries to one per bar.
                // In backtest: HistoryItemUpdated fires once per bar at close. The preceding
                // NewHistoryItem already set newBar=true, so the first (and only) tick fires.
                // In live: NewHistoryItem sets newBar=true; only the first HistoryItemUpdated
                // tick after bar close fires the entry — all subsequent ticks are blocked.
                // GetValue(0) indices are kept identical to the original so backtest results
                // are unchanged.
                if (!this.newBar)
                    return;
                this.newBar = false;
