...
- [Quantower Trading Strategies](#quantower-trading-strategies)
  ...
  - [EMA Cross Strategy](#ema-cross-strategy)
    ...
    - [Exit Logic (controlled by Exit Mode)](#exit-logic-controlled-by-exit-mode)
    - [Parameters](#parameters)
      - [Core](#core)
      - [Risk Management](#risk-management)
      - [Trailing Stop](#trailing-stop)
      - [Session-Aware Trailing Stop](#session-aware-trailing-stop)
...
| **Micro EMA Pullback Touch (ticks)** | 0 | Applies to ALL normal (non-impulse) crosses. When > 0, the strategy does not enter immediately on the cross bar. Instead it waits for price to pull back and come within this many ticks of the Micro EMA (5 EMA), then enters on the first bar that closes back away from it in the cross direction. Useful on the 5m timeframe where price often pushes away from the cross before giving a better entry. Set to 0 to enter immediately on the cross bar close |
